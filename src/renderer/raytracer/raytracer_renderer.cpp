...
void cg::renderer::ray_tracing_renderer::render()
{
	...
	raytracer->miss_shader = [](const ray& ray) {
		...
		payload.color = {0.f, 0.f, 0.f};// CREATIVE TASK
		return payload;
	};
	std::random_device random_device;                                     // CREATIVE TASK
	std::mt19937 random_generator(random_device());                       // CREATIVE TASK
	std::uniform_real_distribution<float> uniform_distribution(-1.f, 1.f);// CREATIVE TASK
	raytracer->closest_hit_shader = [&](const ray& ray, payload& payload, const triangle<cg::vertex>& triangle, size_t depth) {
		...
		cg::renderer::ray to_next_object(position, random_direction);        // CREATIVE TASK
		auto next_payload = raytracer->trace_ray(to_next_object, depth);     // CREATIVE TASK
		result_color += triangle.diffuse * next_payload.color.to_float3() *  // CREATIVE TASK
						std::max(dot(normal, to_next_object.direction), 0.f);// CREATIVE TASK
