...
namespace cg::renderer
{
	...
	inline void rasterizer<VB, RT>::apply_sepia_tone_filter()
	{
		if (!render_target)
			return;
		...
		for (size_t y = 0; y < height; ++y)
		{
			for (size_t x = 0; x < width; ++x)
			{
				...
				float tr = 0.393f * pixel.r + 0.769f * pixel.g + 0.189f * pixel.b;
				float tg = 0.349f * pixel.r + 0.686f * pixel.g + 0.168f * pixel.b;
				float tb = 0.272f * pixel.r + 0.534f * pixel.g + 0.131f * pixel.b;

				// and clamp values to ensure valid range [0, 255]
				pixel.r = static_cast<uint8_t>(std::min(255.0f, tr));
				pixel.g = static_cast<uint8_t>(std::min(255.0f, tg));
				pixel.b = static_cast<uint8_t>(std::min(255.0f, tb));
			}
		}
	}
